    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-50 to-white dark:from-slate-950 dark:to-slate-900 px-4">
      <div className="w-full max-w-sm">
        <div className="mb-6 text-center">
          <a href={WEBSITE_URL} className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 transition-colors">
        ...
        <div className="card">
          {registered ? (
            <div className="text-center py-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-1">Check your email</h2>
              ...
              {verifyUrl && (
                <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 p-3 mb-4 text-left">
                  ...
                  <a href={verifyUrl} className="text-xs text-amber-600 dark:text-amber-300 underline break-all">{verifyUrl}</a>
                </div>
              )}
              <button className="text-sm text-brand-600 dark:text-brand-400 hover:underline" onClick={() => { setRegistered(false); setMode('login'); }}>
                Back to sign in
              </button>
            </div>
          ) : (
            <>
              <div className="flex rounded-xl bg-slate-100 dark:bg-slate-700/50 p-1 mb-6">
                {(['login', 'register'] as const).map((m) => (
                  <button key={m} onClick={() => { setMode(m); setError(''); }}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition capitalize ${mode === m ? 'bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input className="input" type="password" placeholder={mode === 'register' ? 'At least 8 characters' : '••••••••'} value={password} onChange={(e) => setPassword(e.target.value)} minLength={mode === 'register' ? 8 : 6} required />
                </div>
                ...
                {mode === 'login' && (
                  <p className="text-center text-xs text-slate-400 dark:text-slate-500">
                    <Link href="/forgot-password" className="text-brand-600 dark:text-brand-400 hover:underline">Forgot your password?</Link>
                  </p>
                )}
              </form>

              <div className="mt-4 pt-4 border-t border-slate-100 text-center">
                <button className="text-xs text-slate-400 hover:text-brand-600" onClick={() => { setEmail('demo@vitra.app'); setPassword('demo1234'); setMode('login'); }}>
                  Use demo account
                </button>
              </div>
            </>
