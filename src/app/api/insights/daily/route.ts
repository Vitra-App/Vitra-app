...
export async function POST() {
  ...
  const rl = rateLimit(`insight:${session.user.id}`, 10, 60 * 60 * 1000);
  if (!rl.ok) {
  ...
  try {
    ...
    const [profile, todaySummary, weeklySummaries, bloodwork, todayMeals] = await Promise.all([
      ...
      // Fetch actual foods eaten today
