...
export async function sendVerificationEmail(to: string, verifyUrl: string): Promise<void> {
  ...
  if (!apiKey) {
    // Dev fallback — log the link so devs can test without a real key
    console.info(`[verify-email] Verification link for ${to}: ${verifyUrl}`);
    return;
  }
...
<body style="font-family:sans-serif;background:#f8fafc;padding:40px 20px">
  <div style="max-width:480px;margin:0 auto;background:white;border-radius:16px;padding:40px;box-shadow:0 2px 16px rgba(0,0,0,0.08)">
    <h1 style="color:#0f172a;font-size:24px;margin:0 0 8px">Welcome to Vitra 🌱</h1>
    <p style="color:#64748b;font-size:15px;margin:0 0 32px">
      Confirm your email address to activate your account. This link expires in 24 hours.
    </p>
    <a href="${verifyUrl}" style="display:inline-block;background:#22c55e;color:white;text-decoration:none;padding:14px 28px;border-radius:10px;font-weight:600;font-size:15px">
      Verify Email
    </a>
    <p style="color:#94a3b8;font-size:13px;margin:32px 0 0">
      If you didn't create a Vitra account, you can safely ignore this email.
    </p>
  </div>
</body>
</html>`;
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [to],
      subject: 'Verify your Vitra email',
      html,
    }),
  });
